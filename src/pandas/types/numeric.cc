...
#define MAKE_TYPE_SINGLETON(NAME)                                       \
  static const std::shared_ptr<DataType> k##NAME(new NAME##Type());     \
  template <>                                                           \
...
static Status CopyBitmap(const std::shared_ptr<Buffer>& bitmap,
...
static Status AllocateValidityBitmap(int64_t length, std::shared_ptr<Buffer>* out) {
  int64_t nbytes = BitUtil::BytesForBits(length);
  auto buf = std::make_shared<PoolBuffer>();
  RETURN_NOT_OK(buf->Resize(nbytes));
